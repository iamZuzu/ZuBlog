...
// Turns a filename into a clean, URL-safe slug: lowercase, spaces and any
// other non-alphanumeric run collapsed to a single hyphen. This matters
// beyond tidiness — Next.js's static export can fail to resolve dynamic
// route params that contain raw spaces (the page gets pre-rendered as a
// 404 even though the file exists in content/Publish), so filenames with
// spaces in them silently broke. A post's URL can still be pinned exactly
// with an explicit `slug:` field in its frontmatter, which always wins
// over this.
